...
export const useResumeUpload = () => {
  ...
  const validateFile = (file) => {
    ...
    if (
      ...
      return "Unsupported file format. Please upload PDF, DOCX, ZIP, or CSV files.";
    }
    if (file.size > maxSize) return "File size exceeds the 15MB limit.";
    return null;
  };
  ...
  const uploadCandidate = useCallback(async (file) => {
    ...
    try {
      const response = await resumeApi.uploadResumeCandidate(
        file,
        (progressEvent) => {
  ...
  const uploadRecruiterSingle = useCallback(async (file, candidateId) => {
    ...
    try {
      const response = await resumeApi.uploadResumeRecruiterSingle(
        file,
        candidateId,
        (progressEvent) => {
          const percentCompleted = Math.round(
  ...
  const uploadRecruiterBulk = useCallback(async (files) => {
    ...
    try {
      const response = await resumeApi.uploadResumeRecruiterBulk(
        validFiles,
        (progressEvent) => {
          const percentCompleted = Math.round(
            (progressEvent.loaded * 100) / progressEvent.total,
      ...
      setUploadQueue((prev) =>
        prev.map((item) =>
          item.error ? item : { ...item, status: "failed", error: errMsg },
        ),
      );
      throw err;
    } finally {
  ...
  const uploadRecruiterZip = useCallback(async (zipFile) => {
    ...
    try {
      const response = await resumeApi.uploadResumeRecruiterZip(
        zipFile,
        (progressEvent) => {
          const percentCompleted = Math.round(
            (progressEvent.loaded * 100) / progressEvent.total,
      ...
      const errMsg =
        err.response?.data?.message || err.message || "ZIP upload failed";
      setError(errMsg);
      throw err;
    } finally {
      setLoading(false);
  ...
  const uploadRecruiterDataset = useCallback(async (csvFile) => {
    setLoading(true);
    setError(null);
    setUploadProgress(0);

    try {
      const response = await resumeApi.uploadResumeDataset(
        csvFile,
        (progressEvent) => {
          const percentCompleted = Math.round(
            (progressEvent.loaded * 100) / progressEvent.total,
      ...
      const errMsg =
        err.response?.data?.message || err.message || "Dataset upload failed";
      setError(errMsg);
      throw err;
    } finally {
      setLoading(false);
